...
import { SecureAPIKeyStorage } from '../extension';

interface BaseMessage {
    type: string;
}

interface ConfigMessageRequest extends BaseMessage {
    type: 'getConfigs';
}

interface ConfigMessageResponse extends BaseMessage {
    type: 'setConfigs';
    configs: LLMConfigs;
    providers: LLMProviders;
}

interface ModelsMessageRequest extends BaseMessage {
    type: 'getModels';
    provider: string;
    agentName: string;
}

interface ModelsMessageResponse extends BaseMessage {
    type: 'updateModels';
    agentName: string;
    models: string[];
}

interface SaveConfigMessageRequest extends BaseMessage {
    type: 'saveConfig';
    agentName: string;
    config: LLMProviderConfig;
}

type Message = ConfigMessageRequest | ConfigMessageResponse | ModelsMessageRequest | ModelsMessageResponse | SaveConfigMessageRequest;
...
export class LLMSettingsPanel {
    ...
    constructor(
        ...
        this._panel = _panel;
        this.apiKeyStorage = new SecureAPIKeyStorage(context);
     ...
     */
    public static render(extensionUri: vscode.Uri, context: vscode.ExtensionContext): void {
        ...
        webview.onDidReceiveMessage(
            async (message: Message) => {
                try {
                    switch (message.type) {
                        case 'getConfigs':
                            const configs = await vscode.commands.executeCommand<LLMConfigs>('agenta.getLLMConfigs');
                            const providers = await vscode.commands.executeCommand<LLMProviders>('agenta.getLLMProviders');
                            ...
                            if (configs && providers) {
                                ...
                                webview.postMessage({ type: 'setConfigs', configs, providers } as Message);
                            }
                        ...
                        case 'getModels':
                            ...
                            const models = await vscode.commands.executeCommand<string[]>(
                                'agenta.getLLMModels',
                                message.provider
                            );
                            webview.postMessage({
